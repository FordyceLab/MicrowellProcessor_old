...
def rotateImage(imgdata, org_corners):
    ...
    rot_img = skimage.transform.rotate(imgdata, rotation, resize = True, preserve_range = True).astype('uint16')
    return rot_img
...
def readAndRotateImg(srcHandle, oc, subarray_dims, targetHandle = None):
    ...
    org_corners = exp.Device._corners(oc)
    rot_img = rotateImage(srcImg, org_corners).astype('uint16')
    if targetHandle:
...
def writeSubArraySummaryImg(arrayRepo, handle):
    stackData = np.array([a.summary_image('blank').astype('uint16') for a in arrayRepo.flatten().tolist()])
    sa = arrayRepo[0, 0]
